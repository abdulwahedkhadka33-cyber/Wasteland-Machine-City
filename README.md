...
- 「维修站守卫者」为 16 耐久三阶段教学压轴战：Boss 会根据玩家贴脸贪刀、远距离拖延和频繁跳跃自适应选招；液压冲锤成为主要近中距离攻击，磁钳夹击反制近身贪刀，P2/P3 仍保留小维修机、冲击波、电弧、核心光束、落雷、核心脉冲和终段连携；出招时本体短暂压暗且预警在前景显示，地面电流稳定造成伤害，8 HP/5 HP 会强制释放一次教学技能后再解除；每招仍只造成 1 点伤害，死亡时破碎、爆火花、散件飞出并在烟雾中消失。
- 两个可读终端用于交代启动日志和芯片说明。
...
7. Boss 维修大厅：进入后锁门、显示 Boss 血条，练习读液压冲锤、砸地、磁钳夹击、侧向泄压电弧、冲击波、过载电弧、核心光束、落雷和终段连携，并在收招时反击。
