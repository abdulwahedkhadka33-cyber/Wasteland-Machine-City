...
- Boss 数值保持教学压轴但稍难一点：16 耐久，每招 1 点伤害；状态机会采样玩家贴脸贪刀、远距离拖延和频繁跳跃，P1 以液压冲锤/砸地/磁钳夹击自适应选择，侧向泄压电弧只作为低频变化招式；P2/P3 普通自适应招式收招略短，出招更积极但不缩短预警；出招时本体短暂压暗，攻击预警和伤害视觉提升到 Boss 前景。
- Boss 进入 P2 后在 8 HP 强制释放教学技能，先完成一次小维修机召唤和一次过载电弧再恢复常规受击，并按玩家行为选择液压冲锤、贴地冲击波、锁定脚下的过载电弧、低位核心光束和三段顶部落雷；P3 在 5 HP 强制释放一次暴走核心脉冲后再恢复常规受击。P3 低血核心暴走时 V4/V5 裂纹层显现，加入双向核心脉冲、液压冲锤接冲击波、砸地接电弧连携。所有强招会先显示蓝白/红橙/琥珀预警再出伤害，地面电流伤害盒上移扩大以稳定命中玩家下半身，死亡演出约 1.8 秒，包含核心爆闪、外甲裂纹、碎片飞散、火花烟雾和最终隐藏。
...
- 巡逻维修机器人和 Boss 已切到 Enemies/V2 视觉；Boss 额外叠加 V3 外甲、V4 暴走覆盖和 V5 多部件核心/肩甲/磁钳/管线，由状态机驱动 idle、受击、出招压暗、液压冲锤、磁钳夹击、强制技能提示和暴走反馈；玩家攻击只解析核心、主体和肩甲 hurtbox，Boss 招式伤害盒不会被误伤，小机器人全局攻击范围不变。
- 背景使用 V15 连续切片覆盖 x=-12 到 x=181 左右；相机横向跟随、纵向稳定，跳跃不露边。
- 场景动态层包含落灰雾、灯光 halo、蓝色电流、蒸汽、火花、慢转风扇/齿轮、扫描光、V12/V13/V16 背景闪电、燃烧桶火光、旗帜/锁链轻摆、Boss 液压冲锤火花和砸击尘浪。
...
- V5 Boss 多部件：`Assets/Art/Generated/Enemies/V5/`
