...
- Boss 数值保持教学压轴：14 耐久，每招 1 点伤害；状态机会采样玩家贴脸贪刀、远距离拖延和频繁跳跃，P1 以液压冲锤/砸地/磁钳夹击自适应选择，旧横扫只作为稀有兜底招式。
- Boss 进入 P2 后只召唤一次 2 台 1 耐久小维修机，并按玩家行为选择液压冲锤、贴地冲击波、锁定脚下的过载电弧、低位核心光束和三段顶部落雷；P3 低血核心暴走时 V4/V5 裂纹层显现，加入双向核心脉冲、液压冲锤接冲击波、砸地接电弧连携。所有强招会先显示蓝白/红橙/琥珀预警再出伤害，死亡演出约 1.8 秒，包含核心爆闪、外甲裂纹、碎片飞散、火花烟雾和最终隐藏。
...
- 巡逻维修机器人和 Boss 已切到 Enemies/V2 视觉；Boss 额外叠加 V3 外甲、V4 暴走覆盖和 V5 多部件核心/肩甲/磁钳/管线，由状态机驱动 idle、受击、液压冲锤、磁钳夹击和暴走反馈；玩家攻击只解析核心、主体和肩甲 hurtbox，Boss 招式伤害盒不会被误伤。
